...
import com.techject.dragonfly.common.Utils;
...
public class MjpegInputStream extends DataInputStream {
    ...
    public static final int IMAGE_ORIENTATION = 0;
    ...
    private int getEndOfSequenceSimplified(DataInputStream in, byte[] sequence) throws IOException {
        ...
        for(int i=0; i < endPos-startPos ; i++) {
            ...
            if(c == sequence[seqIndex]) {
                seqIndex++;
                if(seqIndex == sequence.length) return headerLen + startPos + i + 1;
            }else{
                seqIndex = 0;
            }
        }
    ...
    public void readMjpegFrame() throws IOException {
        ...
        try {
            ...
            if(contentLengthNew < 0){
                if(DEBUG) Log.w(TAG,"Worst case for finding EOF_MARKER");
                reset();
                contentLengthNew = getEndOfSequence(this, EOF_MARKER);
            }
        }catch (IllegalArgumentException e) {
            if(DEBUG) Log.w(TAG,"IllegalArgumentException in parseContentLength");
            contentLengthNew = getEndOfSequenceSimplified(this, EOF_MARKER);
            ...
            if(DEBUG) Log.w(TAG,"IOException in parseContentLength");
            reset();
            return;
        }
        ...
        if(count++%skip==0){
            try{
                ...
                if(IMAGE_ORIENTATION != 0){ //need to rotate the output
                    //TODO Problem: returned rotated bitmap is immutable
                    tempBmp = Bitmap.createBitmap(tempBmp,0,0,tempBmp.getWidth(),tempBmp.getHeight(),rotate, true);
                }
